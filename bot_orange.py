...
class AutoClicker:
    def __init__(self, root, exe_path=None, bots_data=None):
        ...
        self.root.title("Anna Gold Grind v1.2 до %s" %TIME_DELTA)
        self.root.wm_attributes("-topmost", True) #поверх всех окон
        self.root.wm_attributes('-alpha', 0.85) #прозрачность
    ...
    def choose_exe_file(self):
        ...
        self.exe_file_path = askopenfilename(filetypes=[("EXE files", "*.exe")])
...
def get_time():
    ...
    datetime_obj = datetime.datetime.strptime(time.ctime(response.tx_time), "%a %b %d %H:%M:%S %Y")
    formatted_date = datetime_obj.strftime("%d%m%Y")
    return(formatted_date)


def check_license():
    global LICENSE, TIME_DELTA
    ...
    def check_key(key):
        ...
        md5 = hashlib.md5((date).encode()).hexdigest() #хеш даты
        ...
        if key == out_string:
            ...
            date_str = get_time() #получаю текущее время с сервера
            date_obj = datetime.datetime.strptime(date_str, "%d%m%Y")

            key_date_str = date #время, извлеченное из ключа
            key_date_obj = datetime.datetime.strptime(key_date_str, "%d%m%Y")


            diff = key_date_obj - date_obj #разница во времени
            if diff.days < -1 or diff.days > 31:
                return False
            else:
                plus30_days = datetime.timedelta(days=30)
                delta = key_date_obj + plus30_days
                TIME_DELTA = "%s-%s-%s" %(delta.day, delta.month, delta.year)
                return True
            

            print('разница во времени: ', diff.days) #разница в днях
            
        else:
            return False

        # Проверяем наличие значения "MyValue" в ключе
        # Открываем ключ реестра для чтения
    # os.getenv('LOCALAPPDATA')
    key_file_path = os.path.join(os.getenv('LOCALAPPDATA'), 'key.ini')
...
def license_window():
    ...
    try:
        try:
            ...
            base_path = os.path.abspath(".")
        window.wm_iconbitmap(os.path.join(base_path, "main.ico"))
    except:
        pass

    window.title('введите ключ активации')
    image = tk.PhotoImage(file=os.path.join(base_path, "lock.png"))
    lock_background = tk.Label(window, image=image)
    lock_background.pack()
    ...
    def check_key():
        ...
        if len(key) < 32:
            return
        #извлекаю каждый 4й символ (8 символов - дата подписки)
        date=''
        for i in range(0, len(key), 3):
            date+=(key[i])
        date = date[:-1]
        ...
        if key == out_string:
            ...
            diff = key_date_obj - date_obj #разница во времени
            if diff.days < -1 or diff.days > 31:
                messagebox.showerror(title='Ошибка ключа', message='Данный ключ недействителен!')
                return False
            else:
                key_file_path = os.path.join(os.getenv('LOCALAPPDATA'), 'key.ini')
                ...
                delta = key_date_obj + plus30_days
                TIME_DELTA = "%s-%s-%s" %(delta.day, delta.month, delta.year)
                messagebox.showinfo(title='Успех!', message='Программа активирована до %s!' %TIME_DELTA)
                window.destroy() #закрываю окно
                LICENSE = True
                return True
            
        else:
            messagebox.showerror(title='Ошибка ключа', message='Неправильный ключ активации!')
            return False


    code_entry = tk.Entry(window, width=30, justify="center")
    code_entry.place(x=85, y=230, height=30, width=185)

    ok_btn = tk.Button(window, width=15, text='далее', bg='orange', command=check_key)
    ok_btn.place(x=115, y=270, height=30, width=120)
...
if __name__ == "__main__":
    #print(os.getenv('LOCALAPPDATA')) #получаю путь к локалной appdata
    #print(os.getenv('USERPROFILE')) #путь к папке юзера
